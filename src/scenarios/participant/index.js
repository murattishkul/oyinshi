...
const handleCallbackQuery = async (bot, callbackQuery) => {
  try {
    ...
    const oldStatus = existingParticipant?.status
    ...
    if (!isNewParticipant && oldStatus) {
      const oldEmoji =
        oldStatus === 'GOING' ? '✅' : oldStatus === 'NOT_GOING' ? '❌' : '🤔'
    ...
    // Логирование
    const actionLog = isNewParticipant
      ? 'registered'
      : `changed from ${oldStatus} to ${status}`
    console.log(
      `Participant ${username} (${userId}) ${actionLog} for game ${gameId}`
    )
  } catch (error) {
