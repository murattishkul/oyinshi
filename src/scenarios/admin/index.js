...
// ОБНОВЛЕННАЯ функция запуска опроса с новыми кнопками
async function launchGamePoll (bot, chatId, userId) {
	try {
		...
		// Создаем сообщение с обновленными кнопками
		const pollMessage = await createGamePollMessage(game)
		
		const { createGameKeyboard } = require('../participant/index') // Adjust path as needed
		const keyboard = await createGameKeyboard(game.id, userId)
...
// Хранилище для связи игр с сообщениями
const gameMessages = new Map()

// ОБНОВЛЕННАЯ функция создания сообщения (без списка "не идут" и статистики внизу)
async function createGamePollMessage (game) {
	try {
		...
		// Список участников (только идущие и возможно)
		if (going.length > 0) {
