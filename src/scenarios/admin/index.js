...
const handleAdminScenario = async (bot, chatId, userId, text, username, msg) => {
...
async function loadAndShowAvailableGroups(bot, chatId, userId) {
	try {
		...
		userStates.set(userId, ADMIN_STATES.IDLE);
		gameData.delete(userId);
		bot.sendMessage(chatId, "❌ Ошибка при загрузке списка групп. Попробуйте позже.");
	}
}
...
async function handleMetadataCollection(bot, chatId, userId, text, username) {
	...
	if (!currentGame.date) {
		...
		if (isValidDate(text)) {
			...
			gameData.set(userId, currentGame);
			return bot.sendMessage(chatId, "⏰ Введите время игры (например: 18:00):");
		} else {
			return bot.sendMessage(chatId, "❌ Неверный формат даты. Используйте формат ДД.ММ.ГГГГ (например: 25.12.2024)");
		}
	}
	...
	if (!currentGame.time) {
		...
		if (isValidTime(text)) {
			...
			gameData.set(userId, currentGame);
			return bot.sendMessage(chatId, "👥 Введите количество игроков (например: 10):");
		} else {
			return bot.sendMessage(chatId, "❌ Неверный формат времени. Используйте формат ЧЧ:ММ (например: 18:00)");
		}
	}
	...
	if (!currentGame.playersCount) {
		...
		if (count && count > 0 && count <= 50) {
			...
			return bot.sendMessage(chatId, "❌ Введите корректное количество игроков (от 1 до 50)");
		}
	}
...
async function launchGamePoll(bot, chatId, userId) {
	try {
		...
		const sentMessage = await bot.sendMessage(currentGame.groupId, pollMessage, {
			reply_markup: keyboard,
			parse_mode: 'HTML'
		});
	...
	try {
		...
		if (going.length > 0) {
			...
			going.forEach((participant, index) => {
				const name = participant.firstName || participant.username || 'Неизвестный';
