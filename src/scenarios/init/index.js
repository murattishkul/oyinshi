...
const handleBotStatusChange = async (bot, update) => {
  try {
    ...
    console.log(
      `Bot status change in ${chat.title} (${chat.id}): ${oldStatus} -> ${newStatus}`
    )
...
const handleBotAddedToGroup = async (bot, chat) => {
  try {
    ...
    if (botMember.status === 'administrator') {
      ...
      console.log(
        `Bot added to ${chat.title} but not as admin. Waiting for admin rights.`
      )
    }
  } catch (error) {
